...
export default function MatchingPage() {
  ...
  const [vistaComparativa, setVista]        = useState(false)
  const [puntuacioMin, setPuntuacioMin]     = useState(0)
  const [exportant, setExportant]           = useState(false)

  useEffect(() => {
    Promise.all([assignmentsApi.detall(id), matchingApi.cercar(id, { limit: 50 })])
      .then(([encRes, matchRes]) => { setEncarrec(encRes.data); setCandidats(matchRes.data.candidats || []) })
  ...
  const toggleSeleccio = (cid) => {
    setSeleccionats((prev) => {
      ...
      if (next.has(cid)) { next.delete(cid) } else {
        if (next.size >= 5) { toast(t('matching.maxCompare'), { icon: 'ℹ️' }); return prev }
        next.add(cid)
      }
      return next
    })
  ...
  const handleProposar = async () => {
    if (seleccionats.size === 0) { toast.error(t('matching.selectFirst')); return }
    try {
      const { data } = await matchingApi.proposar(id, Array.from(seleccionats))
      toast.success(t('matching.proposeOk', { n: data.afegits }))
      setSeleccionats(new Set())
    } catch { toast.error(t('matching.proposeErr')) }
  }

  const handleExportarPdf = async () => {
    if (seleccionats.size === 0) { toast.error(t('matching.selectForPdf')); return }
    setExportant(true)
    try {
      const ids = Array.from(seleccionats).join(',')
      const res = await matchingApi.exportarPdf(id, { candidate_ids: ids, nom_client: encarrec?.titol || 'Client' })
      ...
      toast.success(t('matching.pdfOk'))
    } catch { toast.error(t('matching.pdfErr')) }
    ...
    <div>
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">{t('matching.found', { n: candidatsFiltrats.length })}</p>
        </div>
          ...
          {seleccionats.size > 0 && (
            <>
              <button onClick={handleProposar} className="btn-primary">
                ...
                {t('matching.propose', { n: seleccionats.size })}
              </button>
              <button onClick={handleExportarPdf} disabled={exportant} className="btn-secondary disabled:opacity-60">
                <Download size={16} className="inline mr-2" />
                {exportant ? t('matching.exporting') : t('matching.export')}
              </button>
            </>
          )}
          <button
            onClick={() => setVista(!vistaComparativa)}
      ...
      <div className="card mb-6 flex items-center gap-4">
        ...
        <label className="text-sm text-gray-600 flex-shrink-0">
          {t('matching.minScore', { n: puntuacioMin })}
        </label>
        <input type="range" min="0" max="100" step="5" value={puntuacioMin}
          onChange={(e) => setPuntuacioMin(Number(e.target.value))}
          className="flex-1 accent-primary-800" />
        <span className="text-sm text-gray-400 flex-shrink-0">
          {t('matching.results', { n: candidatsFiltrats.length })}
        </span>
      </div>
      ...
      {vistaComparativa && seleccionats.size >= 2 && (
        <div className="card mb-6">
          <h3 className="font-semibold text-gray-800 mb-4">{t('matching.compareTitle')}</h3>
          <RadarChart candidats={candidatsComparativa} />
        </div>
      )}
      {vistaComparativa && seleccionats.size < 2 && (
        <div className="card mb-6 text-center text-gray-500 py-8">{t('matching.compareHint')}</div>
      )}
      ...
      {candidatsFiltrats.length === 0 ? (
        <div className="card text-center py-16">
          ...
          <p className="text-gray-500">{t('matching.noResults')}</p>
        </div>
      ) : (
        <div className="grid lg:grid-cols-2 gap-4">
          {candidatsFiltrats.map((c) => (
            <CandidateCard
              key={c.candidate_id} candidat={c} showScore
